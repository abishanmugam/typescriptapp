...
export function testTscCompileLike(input: TestTscCompileLike) {
  ...
  sys.baseLine = () => {
    ...
    return {
      file: `${
        ts.isBuild(commandLineArgs) ? 'tsbuild' : 'tsc'
...
function makeSystemReadyForBaseline(
  ...
  for (const [program, builderProgram] of programs) {
    ...
    sys.dtsSignaures.set(
      `${toPathWithSystem(
        sys,
        buildInfoPath
      )}.readable.baseline.txt` as ts.Path,
      dtsSignatureData
    );
      ...
      ts.BuilderState.computeDtsSignature(
        ...
        (signature, sourceFiles) => {
          ...
          dtsSignatureData.set(relativeToBuildInfo(resolvedPath), {
            ...
            exportedModules:
              exportedModules &&
              ts.arrayFrom(exportedModules.keys(), relativeToBuildInfo),
          });
        }
      );
    });
  ...
  function writeSectionHeader(section: ts.BundleFileSection) {
    baselineRecorder.WriteLine(
      `${section.kind}: (${section.pos}-${section.end})${
        ...
        section.kind === ts.BundleFileSectionKind.Prepend
...
type ReadableProgramBuildInfoEmitSignature =
  | string
  | [string, ts.EmitSignature | []];
type ReadableProgramBuildInfoFileInfo<T> = Omit<
...
type ReadableProgramMultiFileEmitBuildInfo = Omit<
...
type ReadableProgramBuildInfoBundlePendingEmit = [
  emitKind: ReadableBuilderFileEmit,
  original: ts.ProgramBuildInfoBundlePendingEmit
];
type ReadableProgramBundleEmitBuildInfo = Omit<
...
function generateBuildInfoProgramBaseline(
  ...
  if (buildInfo.program && ts.isProgramBundleEmitBuildInfo(buildInfo.program)) {
    ...
    buildInfo.program?.fileInfos?.forEach(
      (fileInfo, index) =>
        (fileInfos[toFileName((index + 1) as ts.ProgramBuildInfoFileId)] =
    ...
    buildInfo.program?.fileInfos?.forEach(
      (fileInfo, index) =>
        (fileInfos[toFileName((index + 1) as ts.ProgramBuildInfoFileId)] =
          toReadableFileInfo(fileInfo, ts.toBuilderStateFileInfoForMultiEmit))
    );
...
function verifyTscEditDiscrepancies({
  ...
  for (const outputFile of sys.writtenFiles.keys()) {
    ...
    if (ts.isBuildInfoFile(outputFile)) {
      ...
      !ts.fileExtensionIs(outputFile, '.tsbuildinfo.readable.baseline.txt')
    ) {
      verifyTextEqual(
        incrementalBuildText,
        cleanBuildText,
      ...
      if (
        ...
        ts.Debug.assert(
          !isReadableProgramBundleEmitBuildInfo(cleanReadableBuildInfo?.program)
        );
    ...
    buildInfo: JSON.stringify(
      {
        ...
        program: readableBuildInfo.program && {
          ...
          latestChangedDtsFile: readableBuildInfo.program.latestChangedDtsFile
            ? 'FakeFileName'
            : undefined,
        },
...
export function verifyTsc({
  ...
  describe(`tsc ${commandLineArgs.join(
    ...
    before(() => {
      ...
      edits?.forEach(
        ({ edit, caption, commandLineArgs: editCommandLineArgs }, index) => {
    ...
    if (edits?.length) {
      it('tsc invocation after edit and clean build correctness', () => {
        ...
        Harness.Baseline.runBaseline(
          `${
            ts.isBuild(commandLineArgs) ? 'tsbuild' : 'tsc'
          }/${scenario}/${subScenario.split(' ').join('-')}-discrepancies.js`,
