...
import { applyEdit, createBaseline, watchBaseline } from '../tscWatch/helpers';
import {
  createWatchedSystem,
  File,
  libFile,
  ...
  function verifyCancellation(useBuildInfo: boolean, scenario: string) {
    it(scenario, () => {
      ...
      const {
        sys,
        baseline,
        oldSnap: originalSnap,
      } = createBaseline(
        createWatchedSystem([aFile, bFile, cFile, dFile, config, libFile], {
