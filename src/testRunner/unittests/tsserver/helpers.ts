...
function handleLoggerGroup(
  ...
  logger.info = (s) =>
    msg(s, ts.server.Msg.Info, (s) => originalInfo.call(logger, s));
  ...
  function msg(
    ...
    if (!inGroup || firstInGroup)
      s = padStringRight(type + ' ' + seq.toString(), '          ') + s;
...
function nowString(host: TestServerHost) {
  // E.g. "12:34:56.789"
  const d = host.now();
  return `${ts.padLeft(d.getUTCHours().toString(), 2, '0')}:${ts.padLeft(
    d.getUTCMinutes().toString(),
    2,
    '0'
  )}:${ts.padLeft(d.getUTCSeconds().toString(), 2, '0')}.${ts.padLeft(
...
function sanitizeLog(s: string) {
  return s
    ...
    .replace(
      /\"exportMapKey\"\:\s*\"[_$a-zA-Z][_$_$a-zA-Z0-9]*\|\d+\|/g,
...
export function baselineTsserverLogs(
  scenario: string,
  subScenario: string,
  sessionOrService: { logger: Logger }
) {
...
function loggerToTypingsInstallerLog(
  logger: Logger
): ts.server.typingsInstaller.Log | undefined {
  return logger?.loggingEnabled()
    ? {
        isEnabled: ts.returnTrue,
        writeLine: (s) =>
          logger.log(
            `TI:: [${nowString(logger.host!)}] ${sanitizeLog(s)
...
export class TestTypingsInstallerWorker extends ts.server.typingsInstaller
  ...
  constructor(
    ...
    if (log?.isEnabled()) {
      ...
      (installTypingHost as TestSessionAndServiceHost).baselineHost(
    ...
    if (this.log.isEnabled()) {
      this.log.writeLine(
        `TI:: Updated ${typesRegistryPackageName} npm package`
      );
    }
  ...
  executePendingCommands() {
    ...
    for (const action of actionsToRun) {
      if (this.log.isEnabled()) {
        this.log.writeLine(
          `#${action.requestId} with arguments'${JSON.stringify(
            action.packageNames
          )}':: ${action.success}`
        );
      }
  ...
  sendResponse(
    response: ts.server.SetTypings | ts.server.InvalidateCachedTypings
  ) {
    if (this.log.isEnabled()) {
...
export class TestTypingsInstaller<
  T extends TestTypingsInstallerWorker = TestTypingsInstallerWorker
...
export function fileStats(
  nonZeroStats: Partial<ts.server.FileStats>
): ts.server.FileStats {
  return {
    ts: 0,
    tsSize: 0,
...
export class TestServerEventManager {
  ...
  getEvent<T extends ts.server.ProjectServiceEvent>(
  ...
  hasZeroEvent<T extends ts.server.ProjectServiceEvent>(
    eventName: T['eventName']
  ) {
  ...
  assertProjectInfoTelemetryEvent(
    partial: Partial<ts.server.ProjectInfoTelemetryEventData>,
    configFile = '/tsconfig.json'
  ): void {
    assert.deepEqual<ts.server.ProjectInfoTelemetryEventData>(
      this.getEvent<ts.server.ProjectInfoTelemetryEvent>(
        ts.server.ProjectInfoTelemetryEvent
      ),
      {
  ...
  assertOpenFileTelemetryEvent(info: ts.server.OpenFileInfo): void {
    assert.deepEqual<ts.server.OpenFileInfoTelemetryEventData>(
      this.getEvent<ts.server.OpenFileInfoTelemetryEvent>(
        ts.server.OpenFileInfoTelemetryEvent
      ),
      { info }
    );
  }
  assertNoOpenFilesTelemetryEvent(): void {
    this.hasZeroEvent<ts.server.OpenFileInfoTelemetryEvent>(
      ts.server.OpenFileInfoTelemetryEvent
    );
  }
}
...
function patchHostTimeouts(
  ...
  function checkTimeoutQueueLengthAndRun(expected: number) {
    host.baselineHost(
      `Before checking timeout queue length (${expected}) and running`
    );
    originalCheckTimeoutQueueLength.call(host, expected);
    originalRunQueuedTimeoutCallbacks.call(host);
    host.baselineHost(
      `After checking timeout queue length (${expected}) and running`
    );
  }
  ...
  function runQueuedTimeoutCallbacks(timeoutId?: number) {
    host.baselineHost(
      `Before running timeout callback${
        timeoutId === undefined ? 's' : timeoutId
      }`
    );
    originalRunQueuedTimeoutCallbacks.call(host, timeoutId);
    host.baselineHost(
      `After running timeout callback${
        timeoutId === undefined ? 's' : timeoutId
      }`
    );
  }
  ...
  function runQueuedImmediateCallbacks(checkCount?: number) {
    host.baselineHost(
      `Before running immediate callbacks${
        checkCount === undefined ? '' : ` and checking length (${checkCount})`
      }`
    );
...
export class TestSession extends ts.server.Session {
  ...
  public override executeCommand(request: ts.server.protocol.Request) {
    if (this.logger.hasLevel(ts.server.LogLevel.verbose)) {
      ...
      this.logger.info(
        `request:${ts.server.indent(JSON.stringify(request, undefined, 2))}`
      );
    }
    const response = super.executeCommand(request);
    if (this.logger.hasLevel(ts.server.LogLevel.verbose)) {
      this.logger.info(
        `response:${ts.server.indent(JSON.stringify(response, undefined, 2))}`
      );
...
export function createSessionWithEventTracking<
  T extends ts.server.ProjectServiceEvent
>(
  host: TestServerHost,
  eventNames: T['eventName'] | T['eventName'][],
  opts: Partial<TestSessionOptions> = {}
) {
  const events: T[] = [];
...
export function createSessionWithDefaultEventHandler<
  T extends ts.server.protocol.AnyEvent
>(
  host: TestServerHost,
...
export function createProjectService(
  ...
  const cancellationToken =
    options?.cancellationToken || ts.server.nullCancellationToken;
...
export function checkProjectActualFiles(
  ...
  checkArray(
    `${
      ts.server.ProjectKind[project.projectKind]
...
export function protocolTextSpanWithContextFromSubstring({
  ...
  contextText,
  contextOptions,
}: ProtocolTextSpanWithContextFromString): ts.server.protocol.TextSpanWithContext {
...
export function verifyDynamic(service: ts.server.ProjectService, path: string) {
  const info = ts.Debug.checkDefined(
    ...
    `Expected ${path} in :: ${JSON.stringify(
      ts.arrayFrom(service.filenameToScriptInfo.entries(), ([key, f]) => ({
        key,
        fileName: f.fileName,
        path: f.path,
      }))
