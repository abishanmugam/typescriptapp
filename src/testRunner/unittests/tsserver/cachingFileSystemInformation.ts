...
describe('unittests:: tsserver:: CachingFileSystemInformation:: tsserverProjectSystem CachingFileSystemInformation', () => {
  ...
  function logSemanticDiagnostics(
    ...
    projectService.logger.info(
      `getSemanticDiagnostics:: ${file.path}:: ${diags.length}`
    );
    diags.forEach((d) =>
      projectService.logger.info(ts.formatDiagnostic(d, project))
    );
  }
  ...
  describe('WatchDirectories for config file with', () => {
    function verifyWatchDirectoriesCaseSensitivity(
      ...
      it(`watchDirectories for config file with case ${
...
`,
      ...
      baselineTsserverLogs(
        ...
        `npm install works when ${
          timeoutDuringPartialInstallation
            ? 'timeout occurs inbetween installation'
