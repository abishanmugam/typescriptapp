...
describe('unittests:: tsserver:: ConfiguredProjects', () => {
  ...
  it('syntactic features work even if language service is disabled', () => {
    ...
    const { session, events } =
      createSessionWithEventTracking<ts.server.ProjectLanguageServiceStateEvent>(
        host,
        ts.server.ProjectLanguageServiceStateEvent
      );
    ...
    const options = projectService.getFormatCodeOptions(
      f1.path as ts.server.NormalizedPath
    );
    const edits = project
      .getLanguageService()
      .getFormattingEditsForDocument(f1.path, options);
    assert.deepEqual(edits, [
      { span: ts.createTextSpan(/*start*/ 7, /*length*/ 3), newText: ' ' },
    ]);
  });
...
foo();`,
    ...
    assert.equal(
      service.tryGetDefaultProjectForFile(ts.server.toNormalizedPath(fooDts)),
...
describe('unittests:: tsserver:: ConfiguredProjects:: non-existing directories listed in config file input array', () => {
  it('should be tolerated without crashing the server', () => {
    ...
    assert.isTrue(
      inferredProject.containsFile(file1.path as ts.server.NormalizedPath)
    );
    assert.isFalse(
      projectService.configuredProjects
        .get(configFile.path)!
        .containsFile(file1.path as ts.server.NormalizedPath)
    );
  });
...
describe('unittests:: tsserver:: ConfiguredProjects:: when reading tsconfig file fails', () => {
  it('should be tolerated without crashing the server', () => {
    ...
    const { session, events } =
      createSessionWithEventTracking<ts.server.ConfigFileDiagEvent>(
        host,
        ts.server.ConfigFileDiagEvent
      );
    const originalReadFile = host.readFile;
    host.readFile = (f) => {
      return f === configFile.path ? undefined : originalReadFile.call(host, f);
    };
        ...
        data: {
          ...
          diagnostics: [
            ts.createCompilerDiagnostic(
              ts.Diagnostics.Cannot_read_file_0,
              configFile.path
            ),
          ],
        },
