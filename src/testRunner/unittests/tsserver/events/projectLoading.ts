    ...
    createSession: (host: TestServerHost) => {
      ...
      verifyProjectLoadEvents: (
        expected: [
          ...
          ts.server.ProjectLoadingFinishEvent
        ]
      ) => void;
    }
  ) {
...
`,
        ...
        const { service, session, verifyEventWithOpenTs, verifyEvent } =
          createSessionToVerifyEvent(files.concat(aDTs, aDTsMap, bTs, configB));
        verifyEventWithOpenTs(bTs, configB.path, 1);
  ...
  describe('when using event handler', () => {
    verifyProjectLoadingStartAndFinish((host) => {
      const { session, events } = createSessionWithEventTracking<
        ts.server.ProjectLoadingStartEvent | ts.server.ProjectLoadingFinishEvent
      >(host, [
        ts.server.ProjectLoadingStartEvent,
        ts.server.ProjectLoadingFinishEvent,
      ]);
