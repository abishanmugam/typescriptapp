...
describe('unittests:: tsserver:: events:: ProjectsUpdatedInBackground', () => {
  ...
  function createVerifyInitialOpen(
    ...
    verifyProjectsUpdatedInBackgroundEventHandler: (
      events: ts.server.ProjectsUpdatedInBackgroundEvent[]
    ) => void
  ) {
    return (file: File) => {
      ...
      function verifyWithMaxCacheLimit(
        ...
        it(subScenario, () => {
          ...
          const {
            ...
            verifyInitialOpen,
            verifyProjectsUpdatedInBackgroundEventHandler,
          } = createSession(host, createLoggerWithInMemoryLogs(host));
  ...
  describe('when event handler is set in the session', () => {
    ...
    function createSessionWithProjectChangedEventHandler(
      ...
      const { session, events: projectChangedEvents } =
        createSessionWithEventTracking<ts.server.ProjectsUpdatedInBackgroundEvent>(
          host,
      ...
      function eventToString(
        event: ts.server.ProjectsUpdatedInBackgroundEvent
      ) {
      ...
      function eventsToString(
        events: readonly ts.server.ProjectsUpdatedInBackgroundEvent[]
      ) {
        return '[' + ts.map(events, eventToString).join(',') + ']';
      }

      function verifyProjectsUpdatedInBackgroundEventHandler(
        expectedEvents: readonly ts.server.ProjectsUpdatedInBackgroundEvent[]
      ) {
        assert.equal(
  ...
  describe('when event handler is not set but session is created with canUseEvents = true', () => {
    ...
    function createSessionThatUsesEvents(
      ...
      const { session, getEvents, clearEvents } =
        createSessionWithDefaultEventHandler<ts.server.protocol.ProjectsUpdatedInBackgroundEvent>(
          host,
          ts.server.ProjectsUpdatedInBackgroundEvent,
          {
            noGetErrOnBackgroundUpdate,
            logger: logger || createHasErrorMessageLogger(),
          }
        );
      ...
      function verifyProjectsUpdatedInBackgroundEventHandler(
        expected: readonly ts.server.ProjectsUpdatedInBackgroundEvent[]
      ) {
        const expectedEvents: ts.server.protocol.ProjectsUpdatedInBackgroundEventBody[] =
          ts.map(expected, (e) => {
            return {
        ...
        assert.equal(
          events.length,
          expectedEvents.length,
          `Incorrect number of events Actual: ${ts.map(
            events,
            (e) => e.body
