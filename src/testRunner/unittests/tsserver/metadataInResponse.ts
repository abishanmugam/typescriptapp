...
describe('unittests:: tsserver:: with metadata in response', () => {
  ...
  function verifyOutput(
    host: TestServerHost,
    expectedResponse: ts.server.protocol.Response
  ) {
  ...
  describe('With completion requests', () => {
    ...
    it('can pass through metadata when the command returns array', () => {
      ...
      verifyCommandWithMetadata<
        ts.server.protocol.CompletionsRequest,
        readonly ts.server.protocol.CompletionEntry[]
      >(
        session,
        host,
        {
    ...
    it('returns undefined correctly', () => {
      ...
      const session = createSession(host);
      openFilesForSession([aTs], session);
      verifyCommandWithMetadata<ts.server.protocol.CompletionsRequest>(
        session,
        host,
        {
          command: ts.server.protocol.CommandTypes.Completions,
