...
import { createSession, openFilesForSession } from './helpers';
...
function createExportingModuleFile(
  ...
  return {
    path,
    content: ts
      .arrayOf(exportCount, (i) => `export const ${exportPrefix}_${i} = ${i};`)
      .join('\n'),
  };
}
    ...
    },
    {
      ...
      content:
        exportingFiles
          .map(
            (f) =>
              `export * from "./${ts.removeFileExtension(
                ts.convertToRelativePath(
                  f.path,
