...
function verifyDiagnostics(
  ...
  for (let i = 0; i < actual.length; i++) {
    ...
    assert.equal(
      ts.flattenDiagnosticMessageText(actualError.messageText, '\n'),
  ...
  ts.forEach<[string, string, fakes.ParseConfigHost], void>(
    ...
    ([testName, basePath, host]) => {
      function getParseCommandLine(entry: string) {
        const { config, error } = ts.readConfigFile(entry, (name) =>
          host.readFile(name)
        );
        assert(
          config && !error,
      ...
      function getParseCommandLineJsonSourceFile(entry: string) {
        const jsonSourceFile = ts.readJsonConfigFile(entry, (name) =>
          host.readFile(name)
        );
        assert(
        ...
        testFailure('can report errors on circular imports', 'circular.json', [
          {
            ...
            messageText: `Circularity detected while resolving configuration: ${[
              ts.combinePaths(basePath, 'circular.json'),
              ts.combinePaths(basePath, 'circular2.json'),
              ts.combinePaths(basePath, 'circular.json'),
            ].join(' -> ')}`,
          },
        ]);
