...
describe('unittests:: config:: tsconfigParsing:: parseConfigFileTextToJson', () => {
  function assertParseResult(
    jsonText: string,
    expectedConfigObject: { config?: any; error?: ts.Diagnostic[] }
  ) {
  ...
  function assertParseErrorWithExcludesKeyword(jsonText: string) {
    {
      ...
      const parsedCommand = ts.parseJsonConfigFileContent(
        parsed.config,
        ts.sys,
      ...
      assert.isTrue(
        parsedCommand.errors &&
          ...
          parsedCommand.errors[0].code ===
            ts.Diagnostics.Unknown_option_excludes_Did_you_mean_exclude.code
      );
    }
    {
      const parsed = ts.parseJsonText('/apath/tsconfig.json', jsonText);
      const parsedCommand = ts.parseJsonSourceFileConfigFileContent(
        parsed,
        ts.sys,
        'tests/cases/unittests'
      );
  ...
  function getParsedCommandJson(
    ...
    const host: ts.ParseConfigHost = new fakes.ParseConfigHost(
  ...
  function assertParseFileList(
  ...
  ) {
    {
      ...
      assert.isTrue(
        ts.arrayIsEqualTo(parsed.fileNames.sort(), expectedFileList.sort())
      );
    }
    {
  ...
  function assertParseFileDiagnostics(
  ...
  ) {
    {
      ...
      assert.isTrue(
        parsed.errors.filter((e) => e.code === expectedDiagnosticCode).length >
          0,
      ...
      if (!noLocation) {
        assert.isTrue(
          parsed.errors.filter(
            (e) =>
              e.code === expectedDiagnosticCode && e.file && e.start && e.length
          ).length > 0,
          `Expected error code ${expectedDiagnosticCode} to be in ${JSON.stringify(
            parsed.errors
  ...
  it('returns object with error when json is invalid', () => {
    ...
    const expected = ts.createCompilerDiagnostic(
  ...
  it('parses wildcard directories even when parent directories have dots', () => {
    ...
    assert.deepEqual(parsedCommand.wildcardDirectories, {
      '/foo.bar/src': ts.WatchDirectoryFlags.Recursive,
    });
  });
  ...
  it('correctly parses wild card directories from implicit glob when two keys differ only in directory seperator', () => {
    ...
    assert.deepEqual(parsedCommand.wildcardDirectories, {
      '/foo': ts.WatchDirectoryFlags.Recursive,
    });
  });
});
