...
describe('unittests:: config:: tsconfigParsingWatchOptions:: parseConfigFileTextToJson', () => {
  ...
  function verifyWatchOptions(
      ...
      it('with json api', () => {
        ...
        for (const {
          ...
          addToBaseLine(
            baseline,
            json,
            getParsedCommandJson(json, additionalFiles, existingWatchOptions)
          );
        }
    ...
    function addToBaseLine(
      ...
      baseline.push(
        `Input:: ${JSON.stringify(json, /*replacer*/ undefined, ' ')}`
      );
      baseline.push(`Result: WatchOptions::`);
      baseline.push(
        JSON.stringify(parsed.watchOptions, /*replacer*/ undefined, ' ')
      );
    ...
    function runBaseline(subScenario: string, baseline: readonly string[]) {
      Harness.Baseline.runBaseline(
        `config/tsconfigParsingWatchOptions/${subScenario}.js`,
        baseline.join('\n')
      );
    }
  }
