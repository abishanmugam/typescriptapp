...
function testProjectReferences(
  ...
  checkResult: (prog: ts.Program, host: fakes.CompilerHost) => void
) {
  ...
  const vfsys = new vfs.FileSystem(false, {
    files: { '/lib.d.ts': libFile.content },
  });
  files.forEach((v, k) => {
  ...
  const file = ts.parseJsonConfigFileContent(
    config,
    ts.parseConfigHostFromCompilerHostLike(host),
    ts.getDirectoryPath(entryPointConfigFileName),
    {},
    entryPointConfigFileName
  );
  ...
  it('errors when declaration = false', () => {
    ...
    testProjectReferences(spec, '/primary/tsconfig.json', (program) => {
      ...
      assertHasError(
        ...
        errs,
        ts.Diagnostics.Composite_projects_may_not_disable_declaration_emit
      );
    });
  });
  ...
  it("errors when the referenced project doesn't exist", () => {
    ...
    testProjectReferences(spec, '/primary/tsconfig.json', (program) => {
      ...
      assertHasError(
        ...
        errs,
        ts.Diagnostics.File_0_not_found
      );
    });
  });
  ...
  it("errors when a prepended project reference output doesn't exist", () => {
    ...
    testProjectReferences(spec, '/primary/tsconfig.json', (program) => {
      ...
      assertHasError(
        ...
        errs,
        ts.Diagnostics.Output_file_0_from_project_1_does_not_exist
      );
    });
  });
});
  ...
  it('issues a nice error when the input file is missing', () => {
    ...
    testProjectReferences(spec, '/beta/tsconfig.json', (program) => {
      assertHasError(
        ...
        program.getSemanticDiagnostics(),
        ts.Diagnostics.Output_file_0_has_not_been_built_from_source_file_1
      );
    });
  });
