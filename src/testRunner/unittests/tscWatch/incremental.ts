...
describe('unittests:: tsc-watch:: emit file --incremental', () => {
  ...
  function verifyIncrementalWatchEmitWorker(
    ...
    Harness.Baseline.runBaseline(
      `${
        ts.isBuild(argsToPass) ? 'tsbuild/watchMode' : 'tscWatch'
  ...
  describe('module compilation', () => {
    ...
    describe('own file emit with errors', () => {
      ...
      verifyIncrementalWatchEmit({
        ...
        modifyFs: (host) =>
          host.writeFile(file1.path, file1.content.replace('x = 10', 'z = 10')),
      });
      ...
      it('verify that state is read correctly', () => {
        ...
        assert.deepEqual(
          state.semanticDiagnosticsPerFile!.get(libFile.path as ts.Path),
          ts.emptyArray
        );
        assert.deepEqual(
          state.semanticDiagnosticsPerFile!.get(file1.path as ts.Path),
          ts.emptyArray
        );
        assert.deepEqual(
          state.semanticDiagnosticsPerFile!.get(file2.path as ts.Path),
          [
            {
              ...
              category:
                ts.Diagnostics.Type_0_is_not_assignable_to_type_1.category,
...
`;
    ...
    verifyIncrementalWatchEmit({
      ...
      files: () => [
        ...
        {
          path: configFile.path,
          content: JSON.stringify({ compilerOptions: jsxImportSourceOptions }),
        },
      ],
      modifyFs: (host) =>
        host.writeFile(
    ...
    verifyIncrementalWatchEmit({
      ...
      files: () => [
        ...
        {
          path: configFile.path,
          content: JSON.stringify({ compilerOptions: { importHelpers: true } }),
        },
      ],
      modifyFs: (host) => {
