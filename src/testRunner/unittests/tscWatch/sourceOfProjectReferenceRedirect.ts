...
describe('unittests:: tsc-watch:: watchAPI:: with sourceOfProjectReferenceRedirect', () => {
  ...
  function verifyWatch(
    ...
    runWatchBaseline({
      ...
      subScenario: `${subScenario}${
        alreadyBuilt ? ' when solution is already built' : ''
      }`,
  ...
  describe('with simple project', () => {
    verifyScenario(() => {
      ...
      const animalsConfig = getTsBuildProjectFile(
        'demo',
  ...
  describe('when references are monorepo like with symlinks', () => {
    ...
    function verifySymlinkScenarioWorker(
      ...
      verifyScenario(() => {
        const { bPackageJson, aTest, bFoo, bBar, bSymlink, subScenario } =
          packages();
...
`
          ...
          subScenario: `when packageJson has types field${
...
`
          ...
          subScenario: `when referencing file from subFolder${
            scope ? ' with scoped package' : ''
          }`,
        }));
      });
