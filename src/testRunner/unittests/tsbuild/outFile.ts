...
describe('unittests:: tsbuild:: outFile::', () => {
  ...
  function verifyOutFileScenario({
    ...
    if (!ignoreDtsUnchanged) {
      (edits ??= []).push({
        ...
        edit: (fs) =>
          appendText(fs, '/src/first/first_PART1.ts', 'console.log(s);'),
      });
    }
    if (modifyAgainFs) {
      (edits ??= []).push({
  ...
  verifyTscCompileLike(testTscCompileLike, {
    ...
    compile: (sys) => {
      ...
      const builder = ts.createSolutionBuilder(buildHost, ['/src/third'], {
        verbose: true,
      });
  ...
  describe('Prepend output with .tsbuildinfo', () => {
    ...
    describe('Prologues', () => {
      ...
      verifyOutFileScenario({
        ...
        modifyAgainFs: (fs) =>
          addTestPrologue(fs, '/src/first/first_PART1.ts', `"myPrologue5"`),
      });
