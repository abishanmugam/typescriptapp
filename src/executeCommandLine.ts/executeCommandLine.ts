...
export function isBuild(commandLineArgs: readonly string[]) {
  if (
    ...
    const firstOption = commandLineArgs[0]
      .slice(commandLineArgs[0].charCodeAt(1) === CharacterCodes.minus ? 2 : 1)
      .toLowerCase();
...
export function executeCommandLine(
  ...
  if (isBuild(commandLineArgs)) {
    const { buildOptions, watchOptions, projects, errors } = parseBuildCommand(
...
function reportAllStatistics(sys: System, statistics: Statistic[]) {
  ...
  for (const s of statistics) {
    sys.write(
      padRight(s.name + ':', nameSize + 2) +
        padLeft(statisticValue(s).toString(), valueSize) +
        sys.newLine
    );
  }
}
