...
function formatDiagnosticToProtocol(
  ...
  const start = (diag.file &&
    convertToLocation(getLineAndCharacterOfPosition(diag.file, diag.start!)))!; // TODO: GH#18217
  const end = (diag.file &&
    convertToLocation(
...
function getRenameLocationsWorker(
  ...
  perProjectResults.forEach((projectResults, project) => {
    for (const result of projectResults) {
      ...
      if (
        !seen.has(result) &&
        !getMappedLocationForProject(documentSpanLocation(result), project)
      ) {
        results.push(result);
        seen.add(result);
      }
...
function getReferencesWorker(
  ...
  const perProjectResults = getPerProjectReferences(
    ...
    (project, position) => {
      logger.info(
        `Finding references to ${position.fileName} position ${
          position.pos
  ...
  perProjectResults.forEach((projectResults, project) => {
    for (const referencedSymbol of projectResults) {
      ...
      for (const ref of referencedSymbol.references) {
        if (
          !seenRefs.has(ref) &&
          !getMappedLocationForProject(documentSpanLocation(ref), project)
        ) {
          seenRefs.add(ref);
...
export class Session<TMessage = string> implements EventSender {
    ...
    return diagnostics.map(
      (d) =>
        ({
          ...
          endLocation:
            scriptInfo && scriptInfo.positionToLineOffset(d.start! + d.length!),
          reportsUnnecessary: d.reportsUnnecessary,
          reportsDeprecated: d.reportsDeprecated,
  ...
  private findSourceDefinition(
    ...
    if (needsJsResolution) {
      ...
      if (some(jsDefinitions)) {
        ...
        for (const candidate of some(ambientCandidates)
          ...
          if (
            !fileNameToSearch ||
            !ensureRoot(noDtsProject, fileNameToSearch)
          ) {
            continue;
          }
    ...
    function findImplementationFileFromDtsFileName(
      ...
      if (
        ...
        if (
          ...
          const specifier = `${packageName}/${removeFileExtension(
            pathToFileInPackage
          )}`;
          return auxiliaryProject.resolutionCache.resolveSingleModuleNameWithoutWatching(
            specifier,
    ...
    function getAmbientCandidatesByClimbingAccessChain(): readonly {
      ...
      if (
        ...
        return (
          forEachNameInAccessChainWalkingLeft(initialNode, (nameInChain) => {
    ...
    function searchForDeclaration(
      ...
      const matches = FindAllReferences.Core.getTopMostDeclarationNamesInFile(
        declarationName,
        fileToSearch
      );
  ...
  private getReferences(
    ...
    const symbolName = nameSpan
      ? scriptInfo.getSnapshot().getText(nameSpan.start, textSpanEnd(nameSpan))
      : '';
  ...
  private getFormattingEditsAfterKeystroke(
    ...
    if (
      ...
      const { lineText, absolutePosition } =
        scriptInfo.textStorage.getAbsolutePositionAndLineText(args.line);
  ...
  private getFullNavigateToItems(
    ...
    function addItemsForProject(project: Project) {
      ...
      const unseenItems = filter(
        ...
        (item) =>
          tryAddSeenItem(item) &&
          !getMappedLocationForProject(documentSpanLocation(item), project)
      );
  ...
  private handlers = new Map(
    Object.entries<(request: any) => HandlerResponse>({
      ...
      [protocol.CommandTypes.ImplementationFull]: (
        ...
        return this.requiredResponse(
          this.getImplementation(request.arguments, /*simplifiedResult*/ false)
        );
      },
      [protocol.CommandTypes.References]: (
        request: protocol.FileLocationRequest
      ...
      [protocol.CommandTypes.Rename]: (request: protocol.RenameRequest) => {
        return this.requiredResponse(
          this.getRenameLocations(request.arguments, /*simplifiedResult*/ true)
        );
      },
      [protocol.CommandTypes.RenameLocationsFull]: (
        ...
        return this.requiredResponse(
          this.getRenameLocations(request.arguments, /*simplifiedResult*/ false)
        );
      },
      [protocol.CommandTypes.RenameInfoFull]: (
        request: protocol.FileLocationRequest
      ...
      [protocol.CommandTypes.Quickinfo]: (
        ...
        return this.requiredResponse(
          this.getQuickInfoWorker(request.arguments, /*simplifiedResult*/ true)
        );
      },
      [protocol.CommandTypes.QuickinfoFull]: (
        request: protocol.QuickInfoRequest
        ...
        return this.requiredResponse(
          this.getQuickInfoWorker(request.arguments, /*simplifiedResult*/ false)
        );
      },
      ...
      [protocol.CommandTypes.GetOutliningSpansFull]: (
        ...
        return this.requiredResponse(
          this.getOutliningSpans(request.arguments, /*simplifiedResult*/ false)
        );
      },
      ...
      [protocol.CommandTypes.FileReferencesFull]: (
        ...
        return this.requiredResponse(
          this.getFileReferences(request.arguments, /*simplifiedResult*/ false)
        );
      },
      ...
      [protocol.CommandTypes.CompletionDetailsFull]: (
        ...
        return this.requiredResponse(
          this.getCompletionEntryDetails(request.arguments, /*fullResult*/ true)
        );
      },
      [protocol.CommandTypes.CompileOnSaveAffectedFileList]: (
      ...
      [protocol.CommandTypes.Navto]: (request: protocol.NavtoRequest) => {
        return this.requiredResponse(
          this.getNavigateToItems(request.arguments, /*simplifiedResult*/ true)
        );
      },
      [protocol.CommandTypes.NavtoFull]: (request: protocol.NavtoRequest) => {
        return this.requiredResponse(
          this.getNavigateToItems(request.arguments, /*simplifiedResult*/ false)
        );
      },
      ...
      [protocol.CommandTypes.NavTreeFull]: (request: protocol.FileRequest) => {
        return this.requiredResponse(
          this.getNavigationTree(request.arguments, /*simplifiedResult*/ false)
        );
      },
      [protocol.CommandTypes.DocumentHighlights]: (
      ...
      [protocol.CommandTypes.GetCombinedCodeFix]: (
        ...
        return this.requiredResponse(
          this.getCombinedCodeFix(request.arguments, /*simplifiedResult*/ true)
        );
      },
      [protocol.CommandTypes.GetCombinedCodeFixFull]: (
        request: protocol.GetCombinedCodeFixRequest
        ...
        return this.requiredResponse(
          this.getCombinedCodeFix(request.arguments, /*simplifiedResult*/ false)
        );
      },
      [protocol.CommandTypes.ApplyCodeActionCommand]: (
      ...
      [protocol.CommandTypes.GetEditsForRefactor]: (
        ...
        return this.requiredResponse(
          this.getEditsForRefactor(request.arguments, /*simplifiedResult*/ true)
        );
      },
      [protocol.CommandTypes.GetEditsForRefactorFull]: (
        request: protocol.GetEditsForRefactorRequest
      ...
      [protocol.CommandTypes.ToggleLineCommentFull]: (
        ...
        return this.requiredResponse(
          this.toggleLineComment(request.arguments, /*simplifiedResult*/ false)
        );
      },
      ...
      [protocol.CommandTypes.UncommentSelection]: (
        ...
        return this.requiredResponse(
          this.uncommentSelection(request.arguments, /*simplifiedResult*/ true)
        );
      },
      [protocol.CommandTypes.UncommentSelectionFull]: (
        ...
        return this.requiredResponse(
          this.uncommentSelection(request.arguments, /*simplifiedResult*/ false)
        );
      },
...
function positionToLineOffset(
  ...
  return isConfigFile(info)
    ? locationFromLineAndCharacter(info.getLineAndCharacterOfPosition(position))
