  ...
  implements LanguageServiceHost, ModuleResolutionHost
{
  ...
  /** @internal */
  public static async importServicePluginAsync(
    moduleName: string,
    initialDir: string,
    host: ServerHost,
    log: (message: string) => void,
    ...
    log(
      `Dynamically importing ${moduleName} from ${initialDir} (resolved to ${resolvedPath})`
    );
    ...
    if (result.error) {
      const err =
        ...
        JSON.stringify(result.error);
      (logErrors || log)(
        `Failed to dynamically import module '${moduleName}' from ${resolvedPath}: ${err}`
      );
      return undefined;
    }
    return result.module;
  ...
  getGlobalProjectErrors(): readonly Diagnostic[] {
    return (
      filter(this.projectErrors, (diagnostic) => !diagnostic.file) || emptyArray
    );
  }
  ...
  registerFileUpdate(fileName: string) {
    (this.updatedFileNames || (this.updatedFileNames = new Set<string>())).add(
      fileName
    );
  }
  ...
  private updateGraphWorker() {
    ...
    this.hasInvalidatedResolutions =
      this.resolutionCache.createHasInvalidatedResolutions(returnFalse);
          ...
          if (outPath) {
            ...
            this.generatedFilesMap.forEach((watcher, source) => {
              ...
              if (
                ...
                (
                  this.generatedFilesMap as Map<string, GeneratedFileWatcher>
                ).delete(source);
              }
            });
          }
      ...
      for (const file of sourceFiles) {
        strBuilder += `\t${file.fileName}${
          writeFileVersionAndText
            ? ` ${file.version} ${JSON.stringify(file.text)}`
            : ''
        }\n`;
      }
...
export class InferredProject extends Project {
  ...
  override removeRoot(info: ScriptInfo) {
    ...
    if (this._isJsInferredProject && info.isJavaScript()) {
      if (
        every(this.getRootScriptInfos(), (rootInfo) => !rootInfo.isJavaScript())
      ) {
...
export class ConfiguredProject extends Project {
  ...
  enablePluginsWithOptions(
    ...
    this.plugins.length = 0;
    if (!options.plugins?.length && !this.projectService.globalPlugins.length)
      return;
    const host = this.projectService.host;
    if (!host.require && !host.importPlugin) {
      this.projectService.logger.info(
