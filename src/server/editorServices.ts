...
export function forEachResolvedProjectReferenceProject<T>(
  ...
  if (possibleDefaultRef) {
    ...
    const child =
      project.projectService.findConfiguredProjectByProjectName(configFileName);
    if (child) {
...
export class ProjectService {
  ...
  constructor(opts: ProjectServiceOptions) {
    ...
    const log: (s: string) => void =
      watchLogLevel !== WatchLogLevel.None ? (s) => this.logger.info(s) : noop;
        ...
        Debug.assert(
          ...
          () =>
            `${
              ...
              arrayFrom(
                mapDefinedIterator(this.filenameToScriptInfo.values(), (info) =>
                  info.isAttached(project)
                    ? {
    ...
    this.forEachConfigFileLocation(info, (canonicalConfigFilePath) => {
      ...
      if (configFileExistenceInfo) {
        const infoIsRootOfInferredProject =
          configFileExistenceInfo.openFilesImpactedByConfigFile?.get(info.path);
  ...
  private forEachConfigFileLocation(
    ...
    do {
      if (searchInDirectory) {
        ...
        result = action(
          combinePaths(canonicalSearchPath, 'jsconfig.json') as NormalizedPath,
          jsconfigFileName
        );
  ...
  findDefaultConfiguredProject(info: ScriptInfo) {
    ...
    const project =
      configFileName && this.findConfiguredProjectByProjectName(configFileName);
  ...
  watchWildcards(
    ...
    if (exists) {
      ...
      updateWatchingWildcardDirectories(
        ...
        new Map(
          Object.entries(config!.parsedCommandLine!.wildcardDirectories!)
        ),
  ...
  requestEnablePlugin(
    ...
    if (this.host.importPlugin) {
      const importPromise = project.beginEnablePluginAsync(
        pluginConfigEntry,
        searchPaths,
    ...
    project.endEnablePlugin(
      project.beginEnablePluginSync(
        pluginConfigEntry,
        searchPaths,
        pluginConfigOverrides
      )
