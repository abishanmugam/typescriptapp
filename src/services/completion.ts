...
export function getCompletionsAtPosition(
  ...
  if (
    incompleteCompletionsCache &&
    completionKind === CompletionTriggerKind.TriggerForIncompleteCompletions &&
    previousToken &&
    ...
    (context) => {
      const entries = mapDefined(previousResponse.entries, (entry) => {
        ...
        const result =
          ...
          context.tryResolve(
            info,
            !isExternalModuleNameRelative(stripQuotes(origin.moduleSymbol.name))
          );
...
function typeNodeToExpression(
  ...
  switch (typeNode.kind) {
    ...
    case SyntaxKind.ParenthesizedType:
      ...
      return (
        exp &&
        (isIdentifier(exp) ? exp : factory.createParenthesizedExpression(exp))
      );
...
function getPresentModifiers(
  ...
  sourceFile: SourceFile,
  position: number
): { modifiers: ModifierFlags; span?: TextSpan } {
  if (
    !contextToken ||
  ...
  if (isPropertyDeclaration(contextToken.parent)) {
    modifiers |=
      modifiersToFlags(contextToken.parent.modifiers) & ModifierFlags.Modifier;
...
function createObjectLiteralMethod(
  ...
  switch (declaration.kind) {
    ...
    case SyntaxKind.MethodDeclaration: {
      let effectiveType =
        type.flags & TypeFlags.Union && (type as UnionType).types.length < 10
...
function getInsertTextAndReplacementSpanForImportCompletion(
  ...
  const quotedModuleSpecifier = quote(
    sourceFile,
    preferences,
    escapeSnippetText(origin.moduleSpecifier)
  );
...
export function getCompletionEntriesFromSymbols(
  ...
  for (let i = 0; i < symbols.length; i++) {
    ...
    const originalSortText =
      symbolToSortTextMap?.[getSymbolId(symbol)] ?? SortText.LocationPriority;
...
function getCompletionData(
  ...
  if (contextToken) {
    const importStatementCompletionInfo =
  ...
  const isLiteralExpected =
    !tryCast(previousToken, isStringLiteralLike) && !isJsxIdentifierExpected;
  ...
  function tryGetObjectTypeLiteralInTypeArgumentCompletionSymbols():
    ...
    const intersectionTypeNode = isIntersectionTypeNode(typeLiteralNode.parent)
  ...
  function tryGetClassLikeCompletionSymbols(): GlobalsSearch {
    ...
    if (!(classElementModifierFlags & ModifierFlags.Private)) {
      ...
      const baseTypeNodes =
        isClassLike(decl) && classElementModifierFlags & ModifierFlags.Override
  ...
  function isSolelyIdentifierDefinitionLocation(contextToken: Node): boolean {
    ...
    if (
      ...
      if (
        isPreviousPropertyDeclarationTerminated(contextToken, previousToken.end)
      ) {
        return false; // Don't block completions.
      } else if (
      ...
      if (isObjectTypeDeclaration(location)) {
        ...
        if (
          getLineAndCharacterOfPosition(sourceFile, contextToken.getEnd())
            .line !== getLineAndCharacterOfPosition(sourceFile, position).line
        ) {
          return location;
        }
        ...
        return isValidKeyword(contextToken.kind) ||
          ...
          (isIdentifier(contextToken) &&
            isValidKeyword(
              identifierToKeywordKind(contextToken) ?? SyntaxKind.Unknown
            ))
...
function isStaticProperty(symbol: Symbol) {
  return !!(
    symbol.valueDeclaration &&
    getEffectiveModifierFlags(symbol.valueDeclaration) & ModifierFlags.Static &&
...
function getImportStatementCompletionInfo(
  ...
  return {
    ...
    isTopLevelTypeOnly:
      !!tryCast(candidate, isImportDeclaration)?.importClause?.isTypeOnly ||
      !!tryCast(candidate, isImportEqualsDeclaration)?.isTypeOnly,
    couldBeTypeOnlyImportSpecifier:
      !!candidate && couldBeTypeOnlyImportSpecifier(candidate, contextToken),
    replacementSpan:
...
function getSingleLineReplacementSpanForImportCompletionNode(
  ...
  if (!node) return undefined;
  const top =
    findAncestor(node, or(isImportDeclaration, isImportEqualsDeclaration)) ??
    node;
  ...
  const potentialSplitPoint =
    top.kind === SyntaxKind.ImportDeclaration
      ? getPotentiallyInvalidImportSpecifier(top.importClause?.namedBindings) ??
        top.moduleSpecifier
      : top.moduleReference;
