...
export namespace Messages {
  ...
  export const statementOrExpressionExpected: DiagnosticMessage = createMessage(
    ...
    function visit(node: Node) {
      ...
      if (isDeclaration(node)) {
        ...
        if (hasSyntacticModifier(declaringNode, ModifierFlags.Export)) {
          ...
          // Also TODO: GH#19956
          (errors ||= []).push(
            createDiagnosticForNode(node, Messages.cannotExtractExportedEntity)
          );
          return true;
        }
      }
...
function extractFunctionInScope(
  ...
  const typeParametersAndDeclarations = arrayFrom(
    ...
    (type) => ({ type, declaration: getFirstDeclaration(type) })
  );
  ...
  let call: Expression = factory.createCallExpression(
    callThis ? factory.createPropertyAccessExpression(called, 'call') : called,
...
function collectReadsAndWrites(
  ...
  for (let i = 0; i < scopes.length; i++) {
    ...
    if (
      i > 0 &&
      (scopeUsages.usages.size > 0 || scopeUsages.typeParameterUsages.size > 0)
    ) {
