...
function getNewStatementsAndRemoveFromOldFile(
  ...
  if (
    ...
    usage.oldImportsNeededByNewFile.size() === 0
  ) {
  ...
  for (const sourceFile of program.getSourceFiles()) {
    ...
    for (const statement of sourceFile.statements) {
      forEachImportInStatement(statement, (importNode) => {
        if (
          checker.getSymbolAtLocation(moduleSpecifierFromImport(importNode)) !==
          oldFile.symbol
        )
          return;
        ...
        const pathToNewFileWithExtension = resolvePath(
...
function makeImportOrRequire(
  ...
  const pathToNewFile = resolvePath(
    getDirectoryPath(sourceFile.path),
    newFileNameWithExtension
  );
  const pathToNewFileWithCorrectExtension = getModuleSpecifier(
    program.getCompilerOptions(),
    sourceFile,
    sourceFile.path,
    pathToNewFile,
  ...
  if (useEs6Imports) {
    ...
    return makeImportIfNecessary(
      defaultImport,
      specifiers,
      pathToNewFileWithCorrectExtension,
      quotePreference
    );
  } else {
    ...
    return bindingElements.length
      ? (makeVariableStatement(
          ...
          /*type*/ undefined,
          createRequireCall(
            factory.createStringLiteral(pathToNewFileWithCorrectExtension)
          )
  ...
  return flatMap(toMove, (statement) => {
    if (
      ...
      forEachTopLevelDeclaration(statement, (d) =>
        needExport.has(Debug.checkDefined(tryCast(d, canHaveSymbol)?.symbol))
      )
    ) {
...
function inferNewFilename(
  importsFromNewFile: ReadonlySymbolSet,
  movedSymbols: ReadonlySymbolSet
): string {
  return (
    importsFromNewFile.forEachEntry(symbolNameNoDefault) ||
    movedSymbols.forEachEntry(symbolNameNoDefault) ||
    'newFile'
  );
}
...
function filterImport(
  ...
  switch (i.kind) {
    case SyntaxKind.ImportDeclaration: {
      ...
      const namedBindings =
        clause.namedBindings && filterNamedBindings(clause.namedBindings, keep);
