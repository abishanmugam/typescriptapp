...
function formatSpanWorker(
  ...
  if (previousRange! && formattingScanner.getStartPos() >= originalRange.end) {
    ...
    if (tokenInfo && tokenInfo.pos === previousRangeTriviaEnd!) {
      ...
      const parent =
        findPrecedingToken(tokenInfo.end, sourceFile, enclosingNode)?.parent ||
        previousParent!;
      processPair(
        tokenInfo,
  ...
  function getDynamicIndentation(
    ...
    return {
      ...
      recomputeIndentation: (lineAdded, parent) => {
        if (
          SmartIndenter.shouldIndentChildNode(options, parent, node, sourceFile)
        ) {
  ...
  function characterToColumn(
    ...
    for (let i = 0; i < characterInLine; i++) {
      if (
        sourceFile.text.charCodeAt(startLinePosition + i) === CharacterCodes.tab
      ) {
  ...
  function applyRuleEdits(
    ...
    switch (rule.action) {
      ...
      case RuleAction.InsertSpace:
        ...
        if (
          posDelta !== 1 ||
          sourceFile.text.charCodeAt(previousRange.end) !== CharacterCodes.space
        ) {
          recordReplace(
            previousRange.end,
            currentRange.pos - previousRange.end,
