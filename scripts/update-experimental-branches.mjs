...
const triggeredPR =
  process.env.SOURCE_ISSUE || process.env.SYSTEM_PULLREQUEST_PULLREQUESTNUMBER;
...
async function main() {
  ...
  console.log(
    `Performing experimental branch updating and merging for pull requests ${prnums.join(
      ', '
    )}`
  );
  ...
  for (const numRaw of prnums) {
    ...
    if (num) {
      ...
      const inputPR = await gh.pulls.get({
        owner: 'Microsoft',
        repo: 'TypeScript',
        pull_number: num,
      });
  ...
  for (const branchnum of prnums) {
    ...
    if (mergeTree.indexOf(`===${'='}===`) >= 0) {
      // 7 equals is the center of the merge conflict marker
      throw new Error(
        `Merge conflict detected involving PR ${branch} with other experiment`
      );
    }
    // Merge (always producing a merge commit)
    runSequence([['git', ['merge', branch, '--no-ff']]]);
  }
  // Every branch merged OK, force push the replacement `experimental` branch
  runSequence([['git', ['push', '-f', '-u', 'fork', 'experimental']]]);
}
