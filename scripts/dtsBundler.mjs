...
function isInternalDeclaration(node) {
  return /** @type {any} */ (ts).isInternalDeclaration(
    node,
    node.getSourceFile()
  );
}
...
const nullTransformationContext = /** @type {any} */ (ts)
        ...
        if (symbolsConflict(symbolOfNode, symbolInScope)) {
          fail(
            `Declaration at ${nodeToLocation(decl)}\n    references ${
              ...
              symbolOfNode.declarations &&
              nodeToLocation(symbolOfNode.declarations[0])
            },\n    but containing scope contains a symbol with the same name declared at ${
              symbolInScope.declarations &&
              nodeToLocation(symbolInScope.declarations[0])
            }`
          );
        }
      }
