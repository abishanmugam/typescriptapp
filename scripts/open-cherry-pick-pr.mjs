...
const reviewers = process.env.REQUESTING_USER
  ? [process.env.REQUESTING_USER]
...
async function main() {
  ...
  const currentSha = runSequence([['git', ['rev-parse', 'HEAD']]]);
  ...
  const inputPR = (
    await gh.pulls.get({
      pull_number: +process.env.SOURCE_ISSUE,
      owner: 'microsoft',
      repo: 'TypeScript',
    })
  ).data;
    ...
    runSequence([
      [
        ...
        [
          ...
          'add',
          'nonlocal',
          inputPR.base.repo.git_url.replace(/^git:(?:\/\/)?/, 'https://'),
        ],
      ],
    ]);
    remoteName = 'nonlocal';
  ...
  runSequence([['git', ['fetch', remoteName, baseBranchName]]]);
  let logText = runSequence([
    [
      'git',
      [
        'log',
        `${remoteName}/${baseBranchName}..${currentSha.trim()}`,
        `--pretty="%h %s%n%b"`,
        '--reverse',
      ],
    ],
  ]);
  logText = `Cherry-pick PR #${process.env.SOURCE_ISSUE} into ${
...
${logText.trim()}`;
  ...
  const mergebase = runSequence([
    ['git', ['merge-base', `${remoteName}/${baseBranchName}`, currentSha]],
  ]).trim();
  runSequence([
  ...
  const squashSha = runSequence([['git', ['rev-parse', 'HEAD']]]);
  runSequence([
  ...
  const r = await gh.pulls.create({
    ...
    title: `🤖 Pick PR #${process.env.SOURCE_ISSUE} (${inputPR.title.substring(
      0,
      35
    )}${inputPR.title.length > 35 ? '...' : ''}) into ${
      process.env.TARGET_BRANCH
    }`,
    head: `${userName}:${branchName}`,
    base: process.env.TARGET_BRANCH,
    body: `This cherry-pick was triggered by a request on https://github.com/Microsoft/TypeScript/pull/${
      process.env.SOURCE_ISSUE
    }
Please review the diff and merge if no changes are unexpected.${
      produceLKG
        ? ` An LKG update commit is included separately from the base change.`
        : ''
    }
You can view the cherry-pick log [here](https://typescript.visualstudio.com/TypeScript/_build/index?buildId=${
