...
module.exports = createRule({
  ...
  create(context) {
    ...
    const manySpaces = (text, startIndex) =>
      [startIndex, startIndex + 1].every(
        (i) => text.charAt(i) === SPACE_SYMBOL
      );
      ...
      if (text.charAt(0) === OPEN_SYMBOL) {
        ...
        if (manySpaces(text, 1)) {
          context.report({
            ...
            node,
            loc: { column: node.loc.start.column + 1, line: startLine },
          });
        }
      }
      ...
      if (text.charAt(text.length - 1) === CLOSE_SYMBOL) {
        ...
        if (text.charAt(index) !== SPACE_SYMBOL) {
          context.report({
            ...
            node,
            loc: { column: endColumn - 1, line: endLine },
          });
        }
        ...
        if (manySpaces(text, index - 1)) {
          context.report({
            ...
            node,
            loc: { column: endColumn - 2, line: endLine },
          });
        }
      }
    };
