...
function main() {
  ...
  if (process.argv.length !== 4) {
    ...
    console.log(
      `    node ${path.relative(
        __dirname,
        progName
      )} [TypeScript Repo Root] [DefinitelyTyped Repo Root]`
    );
    return;
  }
  ...
  console.log(
    `Resolved DefinitelyTyped Repo Root: '${resolvedDefinitelyTypedRoot}'.`
  );
...
function filePathEndsWith(path, endingString) {
  ...
  return (
    pathLen > extLen &&
    path.substr(pathLen - extLen, extLen).toLocaleLowerCase() ===
      endingString.toLocaleLowerCase()
  );
}
 ...
 */
function importDefinitelyTypedTest(
  tscPath,
  rwcTestPath,
  testCaseName,
  testFiles,
  ...
  const testDirectoryName =
    testCaseName + '_' + Math.floor(Math.random() * 10000 + 1);
    ...
    .exec(
      ...
      (error, stdout, stderr) => {
        ...
        const testCasePath = path.join(
          rwcTestPath,
          'DefinitelyTyped_' + testCaseName + '.json'
        );
...
function importDefinitelyTypedTests(tscPath, rwcTestPath, definitelyTypedRoot) {
  fs.readdir(definitelyTypedRoot, (err, subDirectories) => {
    ...
    subDirectories
      ...
      .filter((i) =>
        fs.statSync(path.join(definitelyTypedRoot, i)).isDirectory()
      )
      .forEach((d) => {
        ...
        fs.readdir(directoryPath, (err, files) => {
          ...
          for (const filePath of files.map((f) =>
          ...
          if (testFiles.length === 0) {
            ...
            if (
              tsFiles.length > 1 &&
              tsFiles.every(
                (t) => filePathEndsWith(t, '.d.ts') && regexp.test(t)
              )
            ) {
              for (const fileName of tsFiles) {
                importDefinitelyTypedTest(
                  tscPath,
                  rwcTestPath,
            ...
            for (const fileName of tsFiles) {
              importDefinitelyTypedTest(
                tscPath,
                rwcTestPath,
                path.basename(fileName, '-tests.ts'),
                [fileName],
                paramFile
              );
            }
